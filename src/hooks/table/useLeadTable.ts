...
const useLeadTable = () => {
  ...
  const [isFollowupModalOpen, setFollowupModalOpen] = useState<boolean>(false);
  const [isFollowupLoading, setFollowupLoading] = useState<boolean>(false);
  ...
  const fetchLeads = async () => {
    try {
      ...
      setLoading(true);
      const { data } = await axios.get<Lead[]>(`${API_URL}/employee-lead`, {
